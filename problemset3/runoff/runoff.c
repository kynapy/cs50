...
bool vote(int voter, int rank, string name) // DONE
{
    for (int k = 0; k < candidate_count; k++)
    {
        ...
        {
            preferences[voter][rank] = k; // Stores the index of the candidate chosen (Tabulate using index later)
            return true;
        }
    }
    return false;
...
{
    // DONE
    for (int i = 0; i < voter_count; i++) // Loop for each voter until last voter
    {
        int j = 0;
